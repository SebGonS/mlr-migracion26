...
```json
{
  ...
  "tipo_receta": "Tenido",
  "articulo_id": 8,
  "articulo": "JERSEY 24/1",
  "peso": 450.00,
  "cantidad": 10,
  "volumen": 3150.00,
  "maquina": { "id": 3, "codigo": "MAQ-03", "nombre": "BRAZOLI (1)" },
  "pasos": [
    { "orden": 1, "paso_id": 5, "paso": "Descrude" },
    { "orden": 2, "paso_id": 8, "paso": "Tenido" }
  ],
  "insumos": [
    {
      "insumo_id": 101,
      "codigo": "COL-ROJO-01",
      "insumo": "Colorante Rojo",
      "cantidad": 2.5,
      "medida": "g/L",
...
**Frontend flow:**
1. Call `generar_receta(paso_id)` to get `insumos` array
2. Show confirmation dialog pre-filled with `cantidad_requerida_kg` per insumo
...
Only on steps with `flg_final = true`. Allowed during EN_PROCESO or after COMPLETADO.
...
| EN_PROCESO | Finalize (with consumption dialog if `requiere_receta`), Register production (if `flg_final`) |
| COMPLETADO | Register production (if `flg_final`), Edit roll assignments (corrections) |
| OMITIDO | View only |
...
- `orden_produccion_paso.flg_final`: boolean — determines if production output is allowed
