...
--   documento_tipo = 'PARTIDA'  →  'entrega'
--
-- Use case: rows in lote and item_movimientos are currently
-- tagged as PARTIDA but should point back to the entrega
-- that originated them.
--
-- Source of new values  : inventario.lote WHERE documento_tipo='entrega' AND documento_id=v_entrega_id
-- Targets to UPDATE     :
...
--   v_entrega_id  : the entrega.id to set AS the new documento_id
...
--                         populate to restrict to specific items (e.g. one entrega covers
--                         only the rib rolls, another covers the regular rolls)
--
-- Steps (in order):
--   1. INSERT into doc.entrega_detalle (item_id, lote_id, cantidad) from the target lotes
--   2. UPDATE inventario.lote              PARTIDA → entrega
--   3. UPDATE inventario.item_movimientos  PARTIDA → entrega
-- ============================================================
...
AND    documento_id   = 681;    -- ← v_entrega_id
*/
SELECT * FROM doc.entrega WHERE correlativo IN ('394','481' )
-- ── EXECUTE BLOCK ─────────────────────────────────────────────
...
DO $$
DECLARE
    v_entrega_id  INT   := 700;             -- ← CHANGE THIS  (new documento_id to set)
    ...
    v_movs_updated      INT;
BEGIN
    RAISE NOTICE 'Reassigning PARTIDA % → entrega % (item filter: %) ...',
        v_partida_id, v_entrega_id,
        CASE WHEN cardinality(v_item_ids) > 0 THEN v_item_ids::TEXT ELSE 'ALL' END;

    -- 1. Insert entrega_detalle rows from the target lotes
    ...
    RAISE NOTICE '  doc.entrega_detalle rows inserted: %', v_grd_inserted;
    ...
    WHERE  m.lote_id        = l.id
      ...
      AND  l.documento_tipo = 'entrega'    -- lote already flipped in step 2
...
-- ── VERIFY ───────────────────────────────────────────────────
-- After running, confirm the rows now point to the entrega:
-- -------------------------------------------------------------
/*
...
SELECT id, entrega_id, item_id, lote_id, ubicacion_id, cantidad
FROM   doc.entrega_detalle
WHERE  entrega_id = 681;  -- ← v_entrega_id
*/
