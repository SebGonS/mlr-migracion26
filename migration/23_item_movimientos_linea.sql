...
-- Populated by crear_orden_servicio; extend to crear_entrega when billing needs it.
-- ═══════════════════════════════════════════════════════════════

ALTER TABLE inventario.item_movimientos
    ADD COLUMN documento_linea_id BIGINT;

-- SELECT distinct entrega FROM partida 
...
COMMENT ON COLUMN inventario.item_movimientos.documento_linea_id
    IS 'Line item within the source document. Polymorphic: points to orden_servicio_detalle.id, entrega_detalle.id, etc. NULL for docs without line-level tracking.';
