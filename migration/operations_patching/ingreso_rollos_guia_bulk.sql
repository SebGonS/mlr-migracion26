...
--      → adds only lote_rollo_detalle + entrega_detalle
--        (movements untouched — reverse+redo if audit trail needs fixing)
--
-- entrega header is created if it doesn't exist yet; reused if it does.
--
...
-- Select partida,entrega,fecha_registro, from vw_partidas_resumen 
-- where estado in ('Pendiente Receta', 'Pendiente Termofijar','Para Programar') 
-- and cliente NOT IN ('Fredy Gaytan','MLR/Rudy', 'Oswaldo','Montes','MLR/Oswaldo','Jimmy','Boston','A&R TEXTILES') --mlr oswarlo incluirlo en ingreso de mlr
-- AND entrega NOT LIKE '%-%'
...
BEGIN
    ...
    IF v_entrega_id IS NULL THEN
        ...
        RAISE NOTICE 'Reusing entrega id=% (%-% tipo=%)',
    ...
    LOOP
        ...
        FOR i IN 1 .. r.n_rollos LOOP
            ...
            VALUES (v_entrega_id, r.item_id, v_lote_id, v_peso_por_rollo)
    ...
    RAISE NOTICE 'Done. entrega_id=%, partida_id=% — % new lote(s), % patched anchor(s)',
        v_entrega_id, v_partida_id, v_count_new, v_count_patched;
END;
$$;
...
--     -- entrega 716 has no detail rows left, safe to drop
--     DELETE FROM doc.entrega WHERE id = 716
--       AND NOT EXISTS (SELECT 1 FROM doc.entrega_detalle WHERE entrega_id = 716);
--     RAISE NOTICE 'entrega deleted';
-- END;
-- $$;
