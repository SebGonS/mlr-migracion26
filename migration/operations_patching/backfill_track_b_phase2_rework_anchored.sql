-- ============================================================================
-- TRACK B · Phase 2 — REWORK-ANCHORED backfill
-- ============================================================================
-- WHAT: 57 legacy gap partidas that Phase 1/1b/3a skipped because they have NO own
--   COMPACTADO partida_paso_ejecucion to anchor production on — but a REWORK CHILD
--   (mes.partida.partida_origen_id = this partida) DOES have one. The parent still holds
--   its pristine raw rolls in stock; production was recorded (structurally) under the
--   rework child. This backfills the parent's dispatch by consuming its pristine rolls,
--   producing dyed output anchored to the CHILD's compactado ejec, and dispatching to
--   the PARENT's venta.
--
-- MODEL (confirmed with user): the PARENT always gets the dispatch; the rework CHILD
--   holds the production. So the dyed output lote is attributed to the child ejec
--   (documento=child's compactado ejec → ejec→paso→partida = child), while the entrega
--   + egress go to the parent's existing venta. The parent therefore still shows "no
--   own output lote" (its output lives under the child) but IS fully dispatched.
--
-- SAFETY (validated): the rework children of these 57 partidas have a compactado ejec
--   but produced NO output — 0 already-dispatched, 0 output-in-stock — so anchoring here
--   creates NO double-egress. The 15 zero-supply rework partidas (parent raw consumed
--   INTO the rework) are DEFERRED — nothing pristine to source, no fabrication.
--
-- Everything else identical to backfill_track_b_phase3a_partial_fill.sql:
--   • WEIGHT BASIS B (inherit lote.cantidad). • PRISTINE double-egress guard.
--   • Cap at supply: dispatch LEAST(supply, demand) (56 supply>=demand + 1 partial);
--     leftover stays in stock, any demand-excess is flagged, never fabricated.
--   • Ledger-first: one entrega per partida → PRIMARY venta. • Historical dating.
--   • Movement-aware scope. • No phantom-egress DELETE (rolls in stock).
--
-- Scope: 57 partidas / 929 rolls. WRITES: item_movimientos, lote + lote_rollo_detalle,
--   entrega + entrega_detalle (linked to EXISTING venta). No venta created, no rolls
--   fabricated.
--
...
own_compactado AS (SELECT DISTINCT pp.partida_id FROM mes.partida_paso pp
...
child_compactado AS (SELECT ch.partida_origen_id AS partida_id, MIN(ppe.id) AS anchor_ejec_id
  FROM mes.partida ch
  JOIN mes.partida_paso pp ON pp.partida_id=ch.id
  JOIN mes.operacion o ON o.id=pp.operacion_id AND o.codigo='COMPACTADO'
  JOIN mes.partida_paso_ejecucion ppe ON ppe.partida_paso_id=pp.id
  WHERE ch.partida_origen_id IS NOT NULL GROUP BY ch.partida_origen_id),
...
p1 AS (
  SELECT s.partida_id, LEAST(u.raw_rolls, d.dispatched) AS rebuild, u.raw_rolls, d.dispatched
  FROM in_scope s
  JOIN child_compactado cc ON cc.partida_id=s.partida_id
  LEFT JOIN own_compactado oc ON oc.partida_id=s.partida_id
  ...
  WHERE oc.partida_id IS NULL)     -- NO own compactado ejec (else handled by Phase 1/1b/3a)
SELECT
  (SELECT count(*) FROM p1)                                              AS partidas_expect_57,
  (SELECT COALESCE(sum(rebuild),0) FROM p1)                             AS rolls_to_backfill_expect_929,
  (SELECT COALESCE(sum(dispatched),0) FROM p1)                          AS demand_billed_expect_931,
  (SELECT count(*) FROM p1 WHERE raw_rolls < dispatched)                AS partial_partidas_expect_1,
  ...
  (SELECT count(DISTINCT pc.partida_id) FROM mes.partida_componente pc JOIN p1 ON p1.partida_id=pc.partida_id
     ...
     JOIN inventario.item_movimiento_tipo t ON t.id=im.item_movimiento_tipo_id AND t.codigo IN ('SERV_EGR','VENTA_EGR')) AS existing_dispatch_expect_0;
...
BEGIN
    ...
    --------------------------------------------------------------------------
    -- Scope: rework-anchored partidas + PRIMARY venta, CHILD anchor ejec, tercero, date.
    --------------------------------------------------------------------------
    ...
    own_compactado AS (SELECT DISTINCT pp.partida_id FROM mes.partida_paso pp
      ...
      JOIN mes.partida_paso_ejecucion ppe ON ppe.partida_paso_id=pp.id),
    child_compactado AS (SELECT ch.partida_origen_id AS partida_id, MIN(ppe.id) AS anchor_ejec_id
      FROM mes.partida ch JOIN mes.partida_paso pp ON pp.partida_id=ch.id
      JOIN mes.operacion o ON o.id=pp.operacion_id AND o.codigo='COMPACTADO'
      JOIN mes.partida_paso_ejecucion ppe ON ppe.partida_paso_id=pp.id
      WHERE ch.partida_origen_id IS NOT NULL GROUP BY ch.partida_origen_id),
    ...
    primary_venta AS (SELECT DISTINCT ON (vd.partida_id) vd.partida_id, vd.venta_id, vd.id AS venta_detalle_id
      ...
      WHERE vd.partida_id IN (SELECT partida_id FROM in_scope)
      ORDER BY vd.partida_id, vd.venta_id, vd.id)
    SELECT s.partida_id, LEAST(u.raw_rolls, d.dispatched) AS dispatched,   -- cap at supply
           (d.fecha)::timestamptz AS fecha_hora,
           cc.anchor_ejec_id, pv.venta_id, pv.venta_detalle_id,
    ...
    WHERE oc.partida_id IS NULL;

    --------------------------------------------------------------------------
    -- The rolls to rebuild: FIRST `dispatched` (=LEAST(supply,demand)) pristine rolls.
    --------------------------------------------------------------------------
    ...
    IF (SELECT count(*) FROM _roll) <> 929
    ...
    -- STEP 1 · PROD_CONSUMO on the in-stock raw roll (doc = CHILD's compactado ejec)
    ...
    -- STEP 2 · dyed child lote (attributed to CHILD ejec) + lote_rollo_detalle
    ...
    -- STEP 4 · outbound entrega — ONE per PARENT partida, linked to PARENT's PRIMARY venta
    ...
    -- STEP 5 · entrega_detalle + dyed-child egress (to PARENT venta)
    ...
    RAISE NOTICE 'Track B Phase 2 (rework-anchored): % raw consumed, % dyed children, % venta-linked entregas.',
...
-- ══════════════════════════════════════════════════════════════════════════════
SELECT
    (SELECT count(*) FROM _roll)                                                       AS raw_touched_expect_929,
...
SELECT
    (SELECT count(*) FROM _ent)                                                              AS entregas_expect_57,
    ...
    (SELECT count(*) FROM doc.entrega_detalle ed WHERE ed.entrega_id IN (SELECT entrega_id FROM _ent)) AS detalle_lines_expect_929,
    ...
    (SELECT count(*) FROM inventario.item_movimientos im
        ...
        WHERE im.documento_tipo='entrega' AND im.documento_id IN (SELECT entrega_id FROM _ent)) AS dyed_egress_moves_expect_929;
...
-- COMMIT;    -- ← after §2: (a) 929/929/929/0, (b) 929/929/929, (c) 57/0/929/0/929, (d)=0, (e)=0
