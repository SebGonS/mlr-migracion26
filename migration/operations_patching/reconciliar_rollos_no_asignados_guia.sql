-- ============================================================
-- RECONCILIAR: dejar los rollos NO ASIGNADOS de una guia igual
--              a una lista objetivo (item_id, n_rollos, peso_total)
--
-- Caso: la guia tiene rollos pendientes (sin asignar a partida)
...
--
-- Anula (solo no asignados):  item_movimientos, guia_remision_detalle,
--                             lote_rollo_detalle, lote (soft-delete)
-- Crea (target):              lote, lote_rollo_detalle, movimiento
--                             de ingreso, guia_remision_detalle
...
--
-- Cada bloque es UNA guia. Hay dos: 1889 y 1881.
-- ============================================================
...
WHERE l.documento_tipo = 'guia_remision'
  AND l.documento_id   = (SELECT id FROM doc.guia_remision WHERE correlativo = '1889')
...
-- ============================================================
-- BLOQUE A — GUIA 1889 -> debe quedar: 16 rib J 20/1
-- ============================================================
DO $$
DECLARE
    v_correlativo TEXT := '1889';   -- <- guia
    v_serie       TEXT := NULL;     -- <- si tu guia identifica por serie, ponla; si no, se ignora
...
BEGIN
    ...
    -- tipo + mov_tipo + tercero/propietario desde la guia existente
    SELECT gr.guia_remision_tipo_id, grt.item_movimiento_tipo_id, gr.tercero_id
    ...
    FROM doc.guia_remision_tipo grt WHERE grt.id = v_tipo_id;
    ...
    -- ---- 1) ANULAR rollos no asignados de esta guia ----
    ...
    IF v_lote_ids IS NOT NULL THEN
        ...
        DELETE FROM doc.guia_remision_detalle      WHERE lote_id = ANY(v_lote_ids);
        ...
        RAISE NOTICE 'Guia % : anulados % rollos no asignados', v_correlativo, v_anulados;
    ELSE
        RAISE NOTICE 'Guia % : no habia rollos no asignados que anular', v_correlativo;
    END IF;
    ...
    FOREACH fila SLICE 1 IN ARRAY v_target LOOP
        ...
        IF v_item_id = 0 THEN
            RAISE EXCEPTION 'Falta poner el item_id real en el target de la guia %', v_correlativo;
        END IF;
        ...
        FOR i IN 1 .. v_n_rollos LOOP
            ...
            VALUES (v_item_id, 'guia_remision', v_guia_id, v_peso_rollo, v_propietario_id, NULL)
            ...
            VALUES (
                ...
                'Reconciliacion no-asignados guia ' || v_correlativo, NULL
            );

            INSERT INTO doc.guia_remision_detalle (guia_remision_id, item_id, lote_id, cantidad)
            VALUES (v_guia_id, v_item_id, v_lote_id, v_peso_rollo)
    ...
    RAISE NOTICE 'Guia % reconciliada. guia_id=%, doc_mov=%', v_correlativo, v_guia_id, v_doc_mov_id;
END;
$$;

-- ============================================================
-- BLOQUE B — GUIA 1881 -> debe quedar: 16 J 20/1 + 3 pqte cuellos J 20/1
...
FROM inventario.lote l
JOIN doc.guia_remision gr ON gr.id = l.documento_id
JOIN item i ON i.id = l.item_id
WHERE l.documento_tipo = 'guia_remision'
