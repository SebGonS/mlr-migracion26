-- ============================================================================
-- TRACK B · Phase 4 — LAST-STEP-ANCHOR backfill (no-compactado partidas)
-- ============================================================================
-- WHAT: 264 legacy gap partidas that Phase 1/1b/3a/2 all SKIPPED because they have NO
--   COMPACTADO partida_paso_ejecucion (own OR rework-child) to anchor on — but they DO
--   have another production step (all 264 have some ejec: 260 own, the rest a child's).
--   They are legacy-dispatched, have a live venta, and hold real pristine raw rolls in
--   stock — i.e. raw-in-stock OVERSTATEMENT (material physically dispatched, still shown
--   as stock). Requiring a COMPACTADO anchor was too strict; this recovers them.
--
-- RULE (confirmed with user): "if no production step of the expected kind exists but it
--   HAS dispatch, wire it to whatever last step we have." So the anchor = the partida's
--   LAST available ejec: MAX(own ejec) if any, else MAX(rework-child ejec). The goal is
--   to have exactly the rolls we should — NO bloat/noise: only real pristine in-stock
--   rolls, capped at supply, excess flagged (never fabricated).
--
-- Mechanic, weight basis, PRISTINE double-egress guard, ledger-first primary-venta
-- linkage, historical dating, movement-awareness, cap-at-LEAST(supply,demand) — ALL
-- identical to backfill_track_b_phase2_rework_anchored.sql. The ONLY difference is the
-- anchor: last available ejec (any operacion) instead of a COMPACTADO ejec.
--
-- SAFETY: same guards. Excludes rib-with-no-weight (those never enter componente),
--   live partidas, and rolls held under reworks (non-pristine). Cap at supply.
--
-- Scope: 264 partidas / 5,001 rolls (262 supply>=demand + 2 partial); 3 rolls flagged.
--   WRITES: item_movimientos, lote + lote_rollo_detalle, entrega + entrega_detalle
--   (linked to EXISTING venta). No venta created, no rolls fabricated.
--
...
own_comp AS (SELECT DISTINCT pp.partida_id FROM mes.partida_paso pp JOIN mes.operacion o ON o.id=pp.operacion_id AND o.codigo='COMPACTADO' JOIN mes.partida_paso_ejecucion ppe ON ppe.partida_paso_id=pp.id),
child_comp AS (SELECT DISTINCT ch.partida_origen_id AS partida_id FROM mes.partida ch JOIN mes.partida_paso pp ON pp.partida_id=ch.id JOIN mes.operacion o ON o.id=pp.operacion_id AND o.codigo='COMPACTADO' JOIN mes.partida_paso_ejecucion ppe ON ppe.partida_paso_id=pp.id WHERE ch.partida_origen_id IS NOT NULL),
...
p1 AS (
  SELECT s.partida_id, LEAST(u.raw_rolls, d.dispatched) AS rebuild, u.raw_rolls, d.dispatched,
         COALESCE(oe.ejec_id, ce.ejec_id) AS anchor_ejec
  FROM in_scope s
  ...
  LEFT JOIN own_comp oc ON oc.partida_id=s.partida_id
  LEFT JOIN child_comp cc ON cc.partida_id=s.partida_id
  LEFT JOIN own_ejec oe ON oe.partida_id=s.partida_id
  LEFT JOIN child_ejec ce ON ce.partida_id=s.partida_id
  WHERE oc.partida_id IS NULL AND cc.partida_id IS NULL)   -- NO compactado ejec (own or child)
SELECT
  (SELECT count(*) FROM p1)                                              AS partidas_expect_264,
  (SELECT COALESCE(sum(rebuild),0) FROM p1)                             AS rolls_to_backfill_expect_5001,
  (SELECT COALESCE(sum(dispatched),0) FROM p1)                          AS demand_billed_expect_5004,
  (SELECT count(*) FROM p1 WHERE raw_rolls < dispatched)                AS partial_expect_2,
  (SELECT count(*) FROM p1 WHERE anchor_ejec IS NULL)                   AS no_anchor_expect_0,
...
BEGIN
    ...
    primary_venta AS (SELECT DISTINCT ON (vd.partida_id) vd.partida_id, vd.venta_id, vd.id AS venta_detalle_id
      ...
      WHERE vd.partida_id IN (SELECT partida_id FROM in_scope)
      ORDER BY vd.partida_id, vd.venta_id, vd.id)
    SELECT s.partida_id, LEAST(u.raw_rolls, d.dispatched) AS dispatched,
    ...
    WHERE oc.partida_id IS NULL AND cc.partida_id IS NULL;
    ...
    IF (SELECT count(*) FROM _roll) <> 5001
       ...
       OR (SELECT count(*) FROM (SELECT raw_lote_id FROM _roll GROUP BY raw_lote_id HAVING count(*)>1) z) <> 0 THEN
        RAISE EXCEPTION 'scope mismatch: n=%, no_ubic=%, no_anchor=%, dup_raw=%',
    ...
    -- STEP 1 · PROD_CONSUMO on the in-stock raw roll (doc = last-step ejec)
    ...
    RAISE NOTICE 'Track B Phase 4 (last-step anchor): % raw consumed, % dyed children, % venta-linked entregas.',
...
-- ══════════════════════════════════════════════════════════════════════════════
SELECT
    (SELECT count(*) FROM _roll)                                                       AS raw_touched_expect_5001,
...
SELECT
    (SELECT count(*) FROM _ent)                                                              AS entregas_expect_264,
    ...
    (SELECT count(*) FROM doc.entrega_detalle ed WHERE ed.entrega_id IN (SELECT entrega_id FROM _ent)) AS detalle_lines_expect_5001,
    ...
    (SELECT count(*) FROM inventario.item_movimientos im
        ...
        WHERE im.documento_tipo='entrega' AND im.documento_id IN (SELECT entrega_id FROM _ent)) AS dyed_egress_moves_expect_5001;
...
-- COMMIT;    -- ← after §2: (a) 5001/5001/5001/0, (b) 5001/5001/5001, (c) 264/0/5001/0/5001, (d)=0, (e)=0
